...
class NoteSetRelationsController extends BaseController {
	...
	public function perm_edit_set_from_body( $req ) {
	...
	public function perm_edit_set_from_param( $req ) {
		$auth = $this->require_logged_in();
		if ( $auth !== true ) return $auth;
	...
	public function perm_note_from_param( $req ) {
		$auth = $this->require_logged_in();
		if ( $auth !== true ) return $auth;
	...
	public function sync( $req ) {
		return $this->safe( function() use ( $req ) {
			$res = $this->repo->sync_set_notes(
				absint( $req['set_id'] ),
				(array) ( $req->get_param( 'note_ids' ) ?: [] )
			);
			return $this->ok( $res );
		});
	}

	public function list_notes_for_set( $req ) {
		return $this->safe( function() use ( $req ) {
	...
	public function list_sets_for_note( $req ) {
		return $this->safe( function() use ( $req ) {
