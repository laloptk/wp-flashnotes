...
class TaxonomyRelationsController extends BaseController
{
    ...
    public function register_routes(): void
    {
        ...
        register_rest_route($this->namespace, '/' . $this->rest_base . '/attach', [[
            ...
            'permission_callback' => [$this, 'perm_object_from_body'],
            'args'                => [
                'object_type'      => ['type'=>'string','enum'=>['set','card','note'],'required'=>true],
                'object_id'        => ['type'=>'integer','minimum'=>1,'required'=>true],
                'term_taxonomy_id' => ['type'=>'integer','minimum'=>1,'required'=>true],
            ],
        ]]);
        ...
        // POST /bulk-attach
        register_rest_route($this->namespace, '/' . $this->rest_base . '/bulk-attach', [[
            'methods'             => 'POST',
            'callback'            => [$this, 'bulk_attach'],
            'permission_callback' => [$this, 'perm_object_from_body'],
            'args'                => [
                'object_type'       => ['type'=>'string','enum'=>['set','card','note'],'required'=>true],
                'object_id'         => ['type'=>'integer','minimum'=>1,'required'=>true],
                'term_taxonomy_ids' => ['type'=>'array','items'=>['type'=>'integer'],'required'=>true],
            ],
        ]]);
        ...
        register_rest_route($this->namespace, '/' . $this->rest_base . '/object/(?P<object_type>set|card|note)/(?P<object_id>\d+)', [[
            'methods'             => 'GET',
            'callback'            => [$this, 'list_terms_for_object'],
            'permission_callback' => [$this, 'perm_object_from_param'],
        ]]);

        // GET /term/{tt_id}/{object_type}
        register_rest_route($this->namespace, '/' . $this->rest_base . '/term/(?P<term_taxonomy_id>\d+)/(?P<object_type>set|card|note)', [[
            'methods'             => 'GET',
            'callback'            => [$this, 'list_objects_for_term'],
            'permission_callback' => [$this, 'require_logged_in'], // simple: must be logged-in to inspect
        ]]);

        // DELETE /object/{object_type}/{object_id}
        register_rest_route($this->namespace, '/' . $this->rest_base . '/object/(?P<object_type>set|card|note)/(?P<object_id>\d+)', [[
            'methods'             => 'DELETE',
            'callback'            => [$this, 'clear_object'],
            'permission_callback' => [$this, 'perm_object_from_param'],
        ]]);
    }
    ...
    public function perm_object_from_body(WP_REST_Request $req)
    {
        ...
        if ($id <= 0) return $this->err('invalid_object', 'Invalid object_id.', 400);
    ...
    public function bulk_attach(WP_REST_Request $req)
    {
        $count = $this->repo->bulk_attach(
            (string) $req['object_type'],
            absint($req['object_id']),
            (array) ($req->get_param('term_taxonomy_ids') ?: [])
        );
        return $this->ok(['inserted' => (int) $count]);
    }
