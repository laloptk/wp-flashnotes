...
class CardSetRelationsController extends BaseController {
	...
	public function perm_card_from_param( $req ) {
		$auth = $this->require_logged_in();
		if ( $auth !== true ) return $auth;
	...
	public function sync( $req ) {
		return $this->safe( function() use ( $req ) {
			$res = $this->repo->sync_set_cards(
				absint( $req['set_id'] ),
				(array) ( $req->get_param( 'card_ids' ) ?: [] )
			);
			return $this->ok( $res );
		});
	}

	public function list_cards_for_set( $req ) {
		return $this->safe( function() use ( $req ) {
	...
	public function list_sets_for_card( $req ) {
		return $this->safe( function() use ( $req ) {
