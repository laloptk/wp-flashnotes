...
class ObjectUsageRepository
{
    ...
    protected array $allowedTypes = ['card' => true, 'note' => true];
    ...
    public function detach(string $object_type, int $object_id, int $post_id, string $block_id): bool
    {
        ...
        $res = $this->wpdb->delete(
            $this->table,
            ['object_type' => $object_type, 'object_id' => $object_id, 'post_id' => $post_id, 'block_id' => $block_id],
            ['%s','%d','%d','%s']
        );
        if ($res === false) {
        ...
        if ($to_remove) {
            foreach (array_chunk($to_remove, 500) as $chunk) {
                ...
                $sql = $this->wpdb->prepare(
                    ...
                    $post_id, $block_id, $object_type, ...$chunk
                );
                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                $res = $this->wpdb->query($sql);
                if ($res === false) {
    ...
    public function clear_object(string $object_type, int $object_id): int
    {
        ...
        $res = $this->wpdb->delete($this->table, ['object_type' => $object_type, 'object_id' => $object_id], ['%s','%d']);
        if ($res === false) {
    ...
    public function clear_post(int $post_id): int
    {
        ...
        $res = $this->wpdb->delete($this->table, ['post_id' => $post_id], ['%d']);
        if ($res === false) {
