...
class TaxonomyRelationsRepository
{
    ...
    protected array $allowedTypes = ['set' => true, 'card' => true, 'note' => true];
    ...
    public function detach(string $object_type, int $object_id, int $tt_id): bool
    {
        ...
        $res = $this->wpdb->delete(
            $this->table,
            ['object_type' => $object_type, 'object_id' => $object_id, 'term_taxonomy_id' => $tt_id],
            ['%s','%d','%d']
        );
        if ($res === false) {
    ...
    public function clear_term(string $object_type, int $tt_id): int
    {
        ...
        $res = $this->wpdb->delete($this->table, ['object_type' => $object_type, 'term_taxonomy_id' => $tt_id], ['%s','%d']);
        if ($res === false) {
